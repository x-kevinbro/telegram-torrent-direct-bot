...
def make_job(user_id: int) -> tuple[int, str, Path]:
    ...
    conn = db()
    cur = conn.execute("INSERT INTO jobs(user_id, token, status, save_path, created_at, expires_at) VALUES(?,?,?,?,?,?)", (user_id, token, "queued", str(save_path), now, now + LINK_EXPIRY_HOURS * 3600))
    conn.commit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not allowed(update):
        ...
        return
    await update.message.reply_text("Send me a legal magnet link or .torrent file. I will download it and return only a temporary direct download link.")
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return
    await update.message.reply_text("\n".join(f"#{r['id']} {r['status']} {int((r['progress'] or 0) * 100)}% {r['name'] or ''}".strip() for r in rows))
